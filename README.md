...
基于 FastAPI + Vue3 的基金净值预测系统，支持 T 日盘中估算、模型训练、回测诊断等功能。
...
针对成立时间较短的新基金（历史数据不足 300 个交易日），系统自动启用**简化训练模式**：

| 特性 | 完整模式 | 简化模式 |
|---|---|---|
| 最少数据要求 | 150 行 | 60 行 |
| 使用模型 | Ridge + ElasticNet + LGBM + XGBoost + Stacking | Ridge + ElasticNet（仅线性模型） |
| Walk-Forward CV | 3-24 轮 | 1-3 轮 |
| 训练窗口 | 250-500 天 | 自适应（约 60% 数据） |
| 预测可靠性 | 高 | 中等（系统会提示） |

**触发条件**: 当基金历史数据在 60-300 行之间时自动启用

**注意事项**: 简化模式下模型预测能力可能受限，建议优先选择成立时间较长的基金进行训练。

## 系统功能

- **T 日盘中估算**: 交易时段内基于持仓映射实时估算当日净值
- **模型训练**: 为指定基金训练专属预测模型（异步任务，含 Walk-Forward CV，支持新基金简化模式）
